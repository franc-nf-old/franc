...
Core tests take longer than any other Franc tests, due to the high amount of computational work involved in validating core components.
...
To run only Franc's core tests (after building):
...
To run only Franc's crypto tests (after building):
...
```
francd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `franc-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Franc's hash tests (after building):
...
To run only Franc's performance tests (after building):
...
To run only Franc's unit tests (after building):
